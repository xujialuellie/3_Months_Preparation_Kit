...
int truckTour(vector<vector<int>> petrolpumps) {
    ...
    int start = 0;
    int end = 1;
    int tem = petrolpumps[start][0] - petrolpumps[start][1];
    while (start != end || tem < 0) {
        while (start != end && tem < 0) {
            tem -= petrolpumps[start][0] - petrolpumps[start][1];
            start = (start + 1) % n;
            if (start == 0) return -1;
        }
        tem += petrolpumps[end][0] - petrolpumps[end][1];
        end = (end + 1) % n;
    }
    return start;
}
